...
class Cluster:
    ...
    def get_matfamily_cluster(
        ...
        if kind == "BGM":
            ms = mixture.BayesianGaussianMixture(
                n_components=clusternum,
        ...
        for c in clusters:
            ...
            matfamily.append(
                "-".join(
                    clusterelements.sort_values("count", ascending=False)[:3][
                        "symbol"
                    ].values
                )
            )
    ...
    def get_year_cluster_list(self, df_cluster, clusters):
        ...
        for c in range(len(clusters)):
            ...
            for y in years:
                ylist[c].append(
                    df_cluster[
                        (df_cluster["year"] <= y) & (df_cluster["cluster"] == c)
                    ]["cluster"].count()
                )
    ...
    def get_matfamily_matcolor(self, df_cluster, matfamily):
        ...
        for idx, mf in enumerate(
            np.array(matfamily)[df_cluster["cluster"].value_counts().index.values]
        ):
        ...
        sort_matfamily = np.array(matfamily)[
    ...
    def get_stack_ad_cluster(
        self, df_data, ad_reliability, clustermodel, clusters, inputsize, tick=20
    ):
        matfamlylist = []
        for i in range(int(ad_reliability.max() / tick) + 1):
            ...
            if sum(relfil) > 0:
                matfamlylist.append(
                    collections.Counter(
                        clustermodel.predict(df_data[relfil].iloc[:, :inputsize])
                    )
                )
