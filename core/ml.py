...
class MLModel:
    ...
    def create_ad_starrydata_models(self, targets, df_train, inputsize, tune_params={}):
        ...
        for target in tqdm(targets):
            ...
            regression.save_model(final_model, model_name = os.path.join(self.model_dirpath,'model_' + target.replace(" ", "_")))
    ...
    def get_errors_targets(self, targets, ad_reliability, df_test_inAD, df_test_outAD, inputsize, tick=20):
        for idx, tg in enumerate(targets):
            ...
            if tg == "Z":
                with open(os.path.join(self.error_dirpath,'mapelist_'+tg+'T.pickle'), 'wb') as f:
                    pickle.dump(mapelist, f)
                with open(os.path.join(self.error_dirpath,'rmslelist_'+tg+'T.pickle'), 'wb') as f:
                    pickle.dump(rmslelist, f)
                with open(os.path.join(self.error_dirpath,'r2list_'+tg+'T.pickle'), 'wb') as f:
                    pickle.dump(r2list, f)
            else:
                with open(os.path.join(self.error_dirpath,'mapelist_'+tg.replace(" ", "_")+'.pickle'), 'wb') as f:
                    pickle.dump(mapelist, f)
                with open(os.path.join(self.error_dirpath,'rmslelist_'+tg.replace(" ", "_")+'.pickle'), 'wb') as f:
                    pickle.dump(rmslelist, f)
                with open(os.path.join(self.error_dirpath,'r2list_'+tg.replace(" ", "_")+'.pickle'), 'wb') as f:
                    pickle.dump(r2list, f)
    ...
    def get_mape_table(self, target, df_reltable, tick):
        mapedlist = []
        with open(os.path.join(self.error_dirpath,'mapelist_'+target.replace(" ", "_")+'.pickle'), 'rb') as f:
